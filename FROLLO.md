...
- **Protocolo canônico atual** vive em `lib/runner/protocol.py` como `frollo.event.v0`: nome do schema, kinds conhecidos, construtor e validação do envelope comum. O Codex já normaliza eventos por esse caminho; o Claude sintetiza eventos v0 em paralelo dentro de `Turn`, mas o renderer Claude ainda usa a máquina antiga diretamente, então a issue do protocolo segue parcial.
...
|---|---|---|
| **Cota de uso (Claude Max)** | `lib/usage.py`, `bin/chat.py` | Thread daemon faz `GET /api/oauth/usage` (o mesmo endpoint que a TUI do Claude Code consome) a cada 5min por default; override via `$FROLLO_USAGE_REFRESH_SECONDS`, com piso de 60s para proteger o endpoint interno. Autentica com o `accessToken` de `~/.claude/.credentials.json`. Devolve `session_pct`/`week_pct`/resets **e** `limits[]` (sessão, semana e cotas por modelo `weekly_scoped`); pinta linha 4 do stats pane sem estado textual de carregamento. O bootstrap do `frollo.sh` deixa o Rio Sena visível por 1s + a tentativa inicial de fetch antes de iniciar o chat. Substituiu o scraping de `claude -p /usage`, que a partir de jul/2026 parou de imprimir as % de sessão/semana (movidas pro componente interativo) — ver seção "cota" do stats pane |
...
| **Backend Codex App Server** | `lib/runner/codex.py`, `lib/runner/protocol.py`, `lib/runner/capabilities.py` | `frollo --backend codex` inicia `codex app-server`, cria thread, envia `turn/start` com `effort=high` e `summary=detailed`, e traduz mensagens, reasoning, comandos, diffs, buscas web, approvals, uso e cota para eventos `frollo.event.v0`. A cota usa `account/rateLimits/read` para snapshot inicial/refresh e também aceita `account/rateLimits/updated` durante turnos; o mesmo polling lê `account/read` para manter o email correto no título do stats pane. O contexto usa `inputTokens` como ocupação da janela (`cachedInputTokens` já está incluído ali; somar de novo duplica e faz a barra chegar a 100% cedo). No thinking pane, summaries streamados aparecem ao vivo, mas `content` de reasoning exposto pelo app-server tem prioridade no item final para evitar ficar só nas frases curtas de ação. É experimental, mas real: o processo Codex é persistente durante a sessão do client. Limites atuais ficam declarados por capability: sem `/model`, sem `--resume`/`/refresh`, sem custo por turno. |
| **Eventos v0 paralelos no Claude** | `lib/runner/turn.py`, `lib/runner/protocol.py` | O backend Claude ainda renderiza pela máquina `Turn`, mas agora também acumula `turn.frollo_events` com envelope `frollo.event.v0`: `turn.started/finished`, `usage.updated`, deltas/completion de texto, reasoning, comandos Bash, mudanças de arquivo e tools genéricas. É o primeiro passo para fazer Claude e Codex alimentarem o mesmo renderer sem trocar a UI de uma vez. |
| **Modelo com versão, effort e advisor** | `bin/chat.py`, `lib/runner/__init__.py`, `lib/runner/codex.py` | A UI preserva versão do modelo (`opus 4.8`, `sonnet 4.6`, `fable 5`) em vez de reduzir tudo só à família. `--model-version` e `/model sonnet 4.6` montam IDs versionados (`claude-sonnet-4-6`); IDs completos continuam aceitos. `--effort`/`/effort` passam `low|medium|high|xhigh|max` ao Claude e também ao `turn/start` do Codex (string livre no app-server); default do Frollo é `high`. `--agent`/`/agent` passam o agente ao Claude, e `--advisor`/`/advisor` são atalhos para `agent=advisor`. Prompt e título do pane exibem modelo+versão, effort e agent/advisor. |
| **Contexto preservado no Ctrl+C** | `bin/chat.py` | Durante cada turno o client abre uma captura local com prompt, resposta parcial renderizada (`_last_response_text`), tools e thinking visível desde o início do turno. Se o usuário aperta Ctrl+C antes do backend emitir `result`/`turn.finished`, essa captura vira `interrupted_context.md` no `RUNDIR` e é anexada automaticamente ao próximo turno real, com instrução para o agente usar só como continuidade. Isso evita que texto já visto no terminal desapareça do contexto quando o subprocesso é morto antes de persistir transcript. No fim de um turno normal o arquivo é limpo. |
...
| `lib/runner/turn.py` | ~420 | Classe `Turn` — máquina de estados do turno (Fase 2): consome linhas do stream-json e despacha por tipo de evento (`_handle_stream_event`, `_handle_content_block_delta`, `_handle_result` etc.); também sintetiza eventos `frollo.event.v0` em `turn.frollo_events` para a migração gradual do renderer |
...
**Ctrl+C durante turno**: cancela o subprocesso, mas preserva o que já estava observável no Frollo
para o próximo turno: prompt interrompido, resposta parcial, tools e thinking visível. O conteúdo fica
em `interrupted_context.md` dentro do `RUNDIR` e é injetado uma vez no próximo prompt enviado ao
backend; depois de um turno normal ele é removido. Ctrl+C enquanto só está lendo input continua apenas
cancelando/limpando a linha.
...
**Thinking omitido / auto-resize**: o pane só cresce quando chega texto de thinking de fato. Blocos `thinking` vazios (só `signature_delta`) são acumulados e a nota fica deferida para o `result`: se algum bloco do turno teve texto, o pane mantém esse thinking e não é sobrescrito por *omitted*; se nenhum teve texto, mostra uma nota final distinguindo `display:omitted` de `thinking_tokens=0`. Isso cobre o comportamento atual do Claude Code 2.1.215, em que a flag `--thinking-display summarized` ainda funciona, mas alguns requests/subrequests legitimamente não têm resumo visível. O `thinking_autoresize` (default `true`) controla o crescimento dinâmico do pane (idle→full→summary).

> **Atualização jul/2026 — thinking funciona em todos os modelos.** A Anthropic mudou o default de `thinking.display` na API pra `"omitted"` em modelos novos (Sonnet 5, Opus 4.8/4.7) — isso quebrou o thinking do Sonnet no Frollo (Haiku continuou funcionando por não estar na leva nova). A crença anterior de que **Opus omite o thinking de forma irrecuperável** (verificado empiricamente em maio/2026: Opus emitia só `signature_delta`) também era, na verdade, só esse mesmo default da API — não uma limitação do modelo. Corrigido forçando `--thinking-display summarized` no spawn do `claude` (ver tabela de features acima); confirmado ao vivo que **Sonnet, Opus, Haiku e Fable** voltaram a mostrar thinking. Não depende mais do default variável da API — qualquer modelo serve pro propósito do Frollo agora, a recomendação de usar especificamente Sonnet não existe mais.
>
...
4. **cota** — no backend Claude, cotas da assinatura Claude Max, com reset. Mostra `sessão` (janela de 5h), `semana` (7d, todos os modelos) e **cotas por modelo** (`weekly_scoped`, ex. `Fable 12%`), coloridas pela `severity` do servidor (ou thresholds de %). Preenchida e atualizada **assincronamente** a cada 5min por default (thread daemon em `chat.py`) via `lib/usage.py` → `GET /api/oauth/usage`; `$FROLLO_USAGE_REFRESH_SECONDS` pode aumentar o intervalo, ou reduzi-lo até o piso de 60s. Enquanto não há dado, a linha fica vazia em vez de mostrar "carregando". No backend Codex, a mesma linha usa `account/rateLimits/read` do app-server para snapshot/refresh e `account/rateLimits/updated` quando o servidor emitir update; o polling também faz `account/read` para trocar o título do stats pane para o email do Codex, sem reutilizar `claude auth status`.

**Mudança do /usage (jul/2026):** o `claude -p /usage` deixou de imprimir as % de sessão/semana — elas passaram a renderizar só no componente Ink interativo; o print virou um resumo textual ("What's contributing to your limits usage?") sem os números. A fonte real é o endpoint `/api/oauth/usage` (`api.anthropic.com`), autenticado com o `accessToken` OAuth de `~/.claude/.credentials.json` + header `anthropic-beta: oauth-2025-04-20`. Retorna JSON com `five_hour`/`seven_day` (`utilization`, `resets_at`) e um `limits[]` já mastigado (`kind` = `session`/`weekly_all`/`weekly_scoped`, `percent`, `severity`, `resets_at`, `scope.model.display_name`). É o mesmo endpoint que a própria TUI consome (`fetchUtilization`). Endpoint interno não-documentado — pode mudar e aparenta ter limite de chamadas; por isso o Frollo usa polling conservador (5min) e mantém piso de 60s mesmo quando configurado por env. Em qualquer falha (401, rede) `fetch_usage()` retorna `None` e o pane mantém a última cota. Como não spawna mais um subprocesso `claude`, sumiu o `--no-session-persistence` (não há mais risco de sessão-zumbi) e o custo caiu de ~1 request de rate-limit + subprocess para um GET de ~200ms.

Preços e tamanhos de janela de contexto por modelo em `runner/stats.py`.

**Seleção de modelo, effort e agent**: `--opus` / `--sonnet` / `--haiku` / `--fable` (shortcuts), `--model <alias|id>` e `--model-version <versão>` na linha de comando, e `/model <nome> [versão]` dentro do chat (ex.: `/model sonnet 4.6` → `claude-sonnet-4-6`). IDs completos continuam aceitos. Sem flag, o Frollo usa **sonnet** por default e `--effort high`. `/effort <low|medium|high|xhigh|max>` troca o effort do próximo turno; no Claude vira `--effort`, no Codex vira o campo `effort` de `turn/start`. `--agent <nome>` e `/agent <nome>` passam o agente ao Claude; `--advisor` e `/advisor` são atalhos para `advisor`. O prompt mostra badges de modelo+versão, effort e agent/advisor à esquerda do modo. O título da borda do pane de chat também fica sempre visível (`▲ chat · sonnet 4.6 · high · advisor`), atualizado no startup, ao fim de cada turno e a cada troca — o cliente seta via `tmux select-pane -T` usando `$TMUX_PANE` (chrome do tmux, não rola com o output).

**Picker de sessões**: `--resume` sem argumento abre picker interativo com histórico de sessões do projeto atual.

**Backend Codex**: `frollo --backend codex` troca o cliente ativo para `codex app-server`. O prompt mostra badge `codex`, o título do pane inclui o backend, modelo observado e effort, reasoning vai para o pane de thinking e comandos/diffs/buscas web aparecem no pane de tools. A linha de cota não reaproveita o cache Claude: ela consulta `account/rateLimits/read` no startup e a cada 5min por default, também aceita os updates esparsos `account/rateLimits/updated`, e formata o reset numérico do app-server como hora/data curta. O título do stats pane começa como `stats · codex` e passa ao email retornado por `account/read` no polling do próprio backend. A barra de contexto é atualizada nos eventos `thread/tokenUsage/updated` e usa `inputTokens` como ocupação total da janela; `cachedInputTokens` aparece como detalhe do turno, mas não entra somado de novo na linha `ctx`. Como capabilities atuais declaram `model_selection=false`, `agent_selection=false` e `session_resume=false`, `--model`, `/model`, `--agent`, `/agent`, `--advisor`, `/advisor`, `--resume` e `/refresh` não tentam simular suporte que o adapter ainda não tem. `--effort`/`/effort` é suportado porque o app-server aceita `effort` no `turn/start`.
...
| `/model [nome] [versão]` | Sem arg: mostra modelo atual. Com arg (`opus`/`sonnet`/`haiku`/`fable`, versão opcional ou ID completo): troca a partir do próximo turno |
| `/effort [nível]` | Sem arg: mostra effort atual. Com arg (`low`/`medium`/`high`/`xhigh`/`max`): troca a partir do próximo turno |
| `/agent [nome]` / `/advisor` | No Claude, mostra/troca o agent; `/advisor` é atalho para `advisor` |
