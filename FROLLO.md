...
4. **cota** — no backend Claude, cotas da assinatura Claude Max, com reset. Mostra `sessão` (janela de 5h), `semana` (7d, todos os modelos) e **cotas por modelo** (`weekly_scoped`, ex. `Fable 12%`), coloridas pela `severity` do servidor (ou thresholds de %). Preenchida e atualizada **assincronamente** a cada 30s (thread daemon em `chat.py`) via `lib/usage.py` → `GET /api/oauth/usage`; enquanto não há dado, a linha fica vazia em vez de mostrar "carregando". No backend Codex, a mesma linha usa `account/rateLimits/read` do app-server para snapshot/refresh e `account/rateLimits/updated` quando o servidor emitir update; o polling também faz `account/read` para trocar o título do stats pane para o email do Codex, sem reutilizar `claude auth status`.
