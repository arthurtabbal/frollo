...
def run_turn(client, message, images=None):
    ...
    try:
            ...
            while ready:
                ...
                if etype == "stream_event":
                    ...
                    if "total_cost_usd" in event:
                        result_cost = event["total_cost_usd"]
                    _r_usage = event.get("usage")
                    if _r_usage:
                        result_in_tok  = _r_usage.get("input_tokens")
                        result_out_tok = _r_usage.get("output_tokens")
        ...
        _disp_input  = result_in_tok  if result_in_tok  is not None else input_tokens
        _disp_output = result_out_tok if result_out_tok is not None else output_tokens
        ...
        if _stats_tty:
            try:
                turn_line = _render_turn_line(_ts(), _disp_input, _disp_output, elapsed, _cost_turn, cache_read_tokens)
